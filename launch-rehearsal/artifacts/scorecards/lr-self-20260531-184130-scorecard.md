...
| Field | Value |
|-------|--------|
| **Run ID** | `lr-self-20260531-184130` |
| **Date** | 2026-05-31 |
...
| `/agents` | Agents — Launch Rehearsal | 1 | 16 | 0 | 433 |
...
| `/init` | Init wizard — Launch Rehearsal | 1 | 17 | 0 | 286 |
| `/integrations` | Integrations — Launch Rehearsal | 1 | 16 | 0 | 124 |
| `/library` | Journey library — Launch Rehearsal | 1 | 18 | 0 | 142 |
| `/recommendations` | Recommendations — Launch Rehearsal | 1 | 22 | 0 | 217 |
| `/runner` | Runner — Launch Rehearsal | 1 | 17 | 0 | 316 |
| `/sitemap` | Site map — Launch Rehearsal | 1 | 16 | 0 | 55 |
| `/trends` | Trends — Launch Rehearsal | 1 | 16 | 0 | 289 |
| `/workflows` | Workflows — Launch Rehearsal | 1 | 16 | 0 | 161 |
...
| `llm-p1-evaluator` | LLM evaluator: First-time evaluator | The dashboard effectively communicates pre-launch readiness with clear metrics a |
| `llm-p2-operator` | LLM evaluator: Daily operator | The Launch Rehearsal Dashboard performs fast and reliably across all core journe |
| `llm-p3-admin` | LLM evaluator: Admin / buyer | Admin persona can navigate core monitoring pages with no errors; all journeys pa |
| `synthesizer` | Cross-agent synthesis | Synthesized 3 issues, 9 delights from 10 agent reports |
...
| UI/UX | 5 | 0 unlabeled buttons; ~1128ms avg step |
...
| **I3** | Init wizard assumes CLI familiarity — The init wizard page displays command-line instructions (e.g., 'rehearse init -c config.yaml && rehearse run -c config.yaml') without an interactive alternative, which may overwhelm first-time evaluators seeking a quick understanding of value. | p1-evaluator | `j4-init-dogfood-p1-evaluator-s1-mobile-seed1-loop1` |
...
| **D4** | Fast interactions for repeat tasks — 57 steps completed under 3s. | p2-operator | `j1-command-center-p1-evaluator-s1-desktop-seed1-loop1` |
| **D5** | Clear readiness score and trend display — The command center shows a prominent 95/100 Green readiness score with blocker and delight counts, along a 41-run trend, instantly communicating product health. | p1-evaluator | `j1-command-center-p1-evaluator-s1-desktop-seed1-loop1` |
| **D6** | Fast page loads — All navigations completed in under 1 second on desktop and tablet, enabling quick task switching. | p2-operator | `j1-command-center-p1-evaluator-s1-desktop-seed1-loop1` |
| **D7** | Clear readiness score with zero blockers — The command center shows a 95/100 readiness score in green with 0 blockers, giving immediate confidence. | p2-operator | `j1-command-center-p1-evaluator-s1-desktop-seed1-loop1` |
| **D8** | Side-by-side run comparison — The compare page allows quick diff of runs with readiness, sitemap, and step outcomes, reducing analysis time. | p2-operator | `j3-compare-p1-evaluator-s1-mobile-seed1-loop1` |
| **D9** | Self-test YAML generation from init wizard — The init wizard offers a one-click 'Generate self-test YAML' and 'Open Runner' to expedite setup. | p2-operator | `j4-init-dogfood-p1-evaluator-s1-mobile-seed1-loop1` |
