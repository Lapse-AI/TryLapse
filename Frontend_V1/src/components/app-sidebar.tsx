...
import {
  LayoutDashboard, Activity, Network, GitBranch, TrendingUp, Bot, Bell,
  Settings, Plug, Terminal, ShieldCheck, GitCompare, Lightbulb, PlayCircle, BookOpen, Wand2,
} from "lucide-react";
      ...
      <div className="px-3 py-3 border-b border-sidebar-border">
        <button className="w-full flex items-center justify-between px-2.5 py-2 rounded-md bg-sidebar-accent hover:bg-sidebar-accent/70 border border-sidebar-border text-left transition-colors">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground font-mono truncate">{workspace?.targetUrl?.replace(/^https?:\/\//, "") ?? "—"}</div>
          </div>
      ...
      <nav className="flex-1 overflow-y-auto py-3">
        {nav.map((group) => (
          <div key={group.group} className="px-3 mb-4">
            <div className="text-[11px] text-muted-foreground px-2 mb-1 font-medium">{group.group}</div>
      ...
      {latest && (
        <div className="border-t border-sidebar-border p-3">
          <div className="rounded-md border border-sidebar-border p-2.5 bg-surface">
            ...
            <Link to="/runs/$runId" params={{ runId: latest.id }} className="mt-0.5 font-mono text-xs text-foreground hover:text-primary block truncate">
              {latest.id}
            </Link>
            <div className="flex items-center gap-1.5 mt-1.5">
              <span className="size-1.5 rounded-full" style={{ background: `var(--${latest.status})` }} />
              <span className="text-[11px] text-muted-foreground">{latest.readiness} · {latest.readinessBand} · {formatRel(latest.startedAt)}</span>
            </div>
          </div>
        </div>
      )}
