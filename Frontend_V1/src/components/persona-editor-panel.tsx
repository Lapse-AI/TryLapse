    ...
    <div className="mt-3 ml-0 rounded-2xl border border-border bg-surface-2/50 p-4 space-y-3">
      ...
      <div className="border-t border-border/50 pt-3">
        <div className="text-[11px] font-medium text-muted-foreground mb-2">
          Behavioral profile{" "}
          <span className="font-normal opacity-60">(shapes journey generation)</span>
        </div>
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="text-[10px] text-muted-foreground">Tech literacy</label>
            <select
              className="mt-0.5 w-full text-xs bg-surface border border-border rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary/30"
              value={techLiteracy}
        ...
        <div className="mt-2">
          <label className="text-[10px] text-muted-foreground">
            Character{" "}
            <span className="opacity-60">e.g. "anxious about billing, reads every tooltip"</span>
          </label>
          <input
            className="mt-0.5 w-full text-xs bg-surface border border-border rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary/30"
            value={character}
            onChange={(e) => setCharacter(e.target.value)}
            placeholder="free-text psychological texture"
          />
        </div>
        <div className="mt-2">
          <label className="text-[10px] text-muted-foreground">
            ...
            <span className="opacity-60">
              e.g. "first-time user", "switching from Competitor X"
            </span>
          </label>
          <input
            className="mt-0.5 w-full text-xs bg-surface border border-border rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary/30"
            ...
            placeholder="context this persona brings"
          />
        </div>
      </div>
    ...
    <div className="col-span-full rounded-2xl border border-border bg-surface p-4 space-y-4">
      ...
      <div className="space-y-2">
        <label className="text-[11px] text-muted-foreground">
          Describe a user type — AI drafts the persona
        </label>
...
export function PersonaEditorPanel({ configId, live, onPersonasChanged, refreshKey }: Props) {
  ...
  useEffect(() => {
    void load();
  }, [configId, live, refreshKey]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="flex items-center gap-2">
          {personas.length > 0 && (
            <span className="text-[11px] text-muted-foreground">
              {personas.length} persona{personas.length !== 1 ? "s" : ""} in config
            </span>
          )}
          ...
          <div key={p.id} className="space-y-0">
            ...
            >
              ...
              <div className="text-center w-full min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground truncate mt-0.5" title={p.role}>
                  {p.role}
                </div>
                {p.goals.length > 0 && (
              ...
              {/* Hover indicator */}
              <div
                className={`text-[10px] transition-opacity duration-150 ${expanded === p.id ? "text-primary/60 opacity-100" : "text-muted-foreground/30 opacity-0 group-hover:opacity-100"}`}
              >
