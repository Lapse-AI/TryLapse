      ...
      <div className="p-8 max-w-[1400px] space-y-6">
        ...
        <Panel className="p-6">
          <div className="text-xs text-muted-foreground">
            Readiness by dimension — {readinessTrend.length} runs
          </div>
          <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-6">
            {bundle.dimensions.map((d, i) => {
              const seed = readinessTrend.map((v, j) =>
                Math.max(40, Math.min(100, v + ((i * 3 + j) % 7) - 3)),
              );
              const tone =
                d.score >= 85 ? "var(--ready)" : d.score >= 75 ? "var(--warn)" : "var(--danger)";
              return (
                <div key={d.name}>
