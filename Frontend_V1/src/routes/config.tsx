...
function Config() {
  ...
  return (
    <div>
      <PageHeader eyebrow="configure" title="Workspace · rehearse.yaml" description="Versioned config. Secrets only via env vars — never in YAML or scorecards." />
      <div className="p-8 max-w-[1400px] space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Panel className="p-5">
            <div className="text-xs text-muted-foreground">Workspace</div>
            <div className="font-display text-xl font-semibold mt-1">{ws.name}</div>
            <div className="text-xs text-muted-foreground font-mono mt-1">{ws.id} · {ws.members} members · {ws.products} products</div>
          </Panel>
          <Panel className="p-5">
            <div className="text-xs text-muted-foreground">Config version</div>
            <div className="font-display text-xl font-semibold mt-1">{ws.configVersion} · {ws.configHash}</div>
          ...
          <Panel className="p-5">
            ...
            <div className="text-xs text-muted-foreground font-mono mt-1 flex items-center gap-2">
              PII redaction:
              <button type="button" onClick={() => save.mutate({ piiRedaction: !ws.piiRedaction })} className="underline">
          ...
          <Panel className="p-5">
            ...
            <div className="font-display text-xl font-semibold mt-1 text-ready">{ws.strictEnterpriseMode ? "enabled" : "off"}</div>
            <div className="text-xs text-muted-foreground font-mono mt-1">expects pricing · docs · admin paths</div>
          </Panel>
        </div>
        ...
        <Panel className="overflow-hidden">
          <div className="p-4 border-b border-border flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" className="text-xs font-mono px-3 py-1 rounded border border-border hover:bg-surface-2">validate</button>
              <button type="button" className="text-xs font-mono px-3 py-1 rounded bg-primary text-primary-foreground">save · v15</button>
            </div>
          </div>
          <pre className="p-5 text-[12.5px] font-mono leading-relaxed overflow-x-auto bg-surface-2/30 text-foreground/95">{yaml}</pre>
        </Panel>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {personas.map((p) => (
                <div key={p.id} className="border border-border rounded-lg p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Chip tone={p.patience === "low" ? "danger" : p.patience === "medium" ? "warn" : "ready"}>{p.patience} patience</Chip>
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">{p.role} — goal: {p.goal}</div>
                  {p.stressFactors && <div className="text-[10px] font-mono text-muted-foreground mt-1">stress: {p.stressFactors.join(", ")}</div>}
                </div>
              ))}
            </div>
          </Panel>
          ...
          <Panel className="p-5 space-y-5">
            <div>
              <div className="text-xs text-muted-foreground mb-3">Environment secrets · REHEARSE_*</div>
              <div className="space-y-2 font-mono text-sm">
                {["REHEARSE_EMAIL", "REHEARSE_PASSWORD", "NVIDIA_NIM_API_KEY", "OPENAI_API_KEY"].map((s) => (
                  <div key={s} className="flex items-center justify-between border border-border rounded-md px-3 py-2">
