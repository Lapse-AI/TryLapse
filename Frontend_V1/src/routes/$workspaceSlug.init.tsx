...
export const Route = createFileRoute("/$workspaceSlug/init")({
    ...
    <Panel className="p-6 flex flex-wrap items-center justify-between gap-4 border-primary/20 bg-primary/3">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          ...
          <span className="font-mono text-foreground text-xs">{workspace.targetUrl}</span>. Start
          the TryLapse CLI then hit run.
        </p>
  ...
  return (
    <div>
      <PageHeader
        ...
        title={
          userWorkspace
            ? `Set up ${userWorkspace.productName || userWorkspace.name}`
            : "rehearse init wizard"
        }
        description={
          userWorkspace
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {localhostTarget && !preflightNeedsLocalhost && (
                <span className="text-xs text-amber-600">
                  Enable allow localhost for local URLs
                </span>
              )}
            </div>
                  ...
                  <>
                    ...
                    <div className="text-xs font-mono text-muted-foreground/70">
                      ...
                      <span className="select-all">
                        ./rehearse run -c launch-rehearsal/artifacts/configs/{workspaceConfigId}
                        .yaml -o launch-rehearsal/artifacts/
                      </span>
                    </div>
                  </>
                ) : (
                  <div>
                    Saves to: <code className="text-xs font-mono">configs/[slug].yaml</code> — slug
                    derived from target URL
                  </div>
                )}
              </div>
            </div>
