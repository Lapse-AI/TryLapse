...
export const Route = createFileRoute("/$workspaceSlug/dashboard")({
    ...
    <div className="flex-1 flex items-center justify-center p-12">
      <div className="max-w-md w-full text-center">
        ...
        <h2 className="font-display text-2xl font-semibold mb-2">
          {hasJourneys ? "Run your first rehearsal" : "Set up your journeys first"}
        </h2>
        <p className="text-sm text-muted-foreground mb-8">
          {hasJourneys ? (
            <>
              TryLapse will send synthetic personas through{" "}
              <span className="font-mono text-foreground text-xs">
                {ws?.targetUrl ?? "your product"}
              </span>{" "}
              and score its readiness across every journey.
            </>
          ) : (
            "Use the setup page to analyze your product, configure personas, and discover journeys before running."
          )}
        </p>
        {hasJourneys ? (
          <button
          ...
          >
            ...
            {isPending ? "Queuing run…" : "Run rehearsal"}
          </button>
        ) : (
          <div className="flex flex-col items-center gap-3">
            ...
            >
              {isPending ? "Queuing…" : "Skip setup and run anyway"}
            </button>
          </div>
        )}
      </div>
